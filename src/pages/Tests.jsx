...
const reportTimeOptions = ["Same day", "24 hours", "48 hours", "3-5 days"];
const popularOptions = ["Most Booked", "Doctor Recommended", "Best Value"];
...
export default function Tests() {
  ...
  const filteredTests = useMemo(() => {
    ...
    if (sampleTypes.length) {
      list = list.filter((test) => sampleTypes.includes(test.sampleType));
    }

    if (fastingRequired !== null) {
      list = list.filter(
        (test) => getFastingFlag(test.fasting) === fastingRequired
      );
    }

    if (reportTimes.length) {
      list = list.filter((test) =>
        reportTimes.some((time) => test.reportTime.includes(time))
      );
    }

    if (popularTags.length) {
      list = list.filter((test) =>
        popularTags.some((tag) => test.tags?.includes(tag))
      );
    }
          ...
          <div className="mt-6 md:mt-8 max-w-2xl">
            <div className="flex items-center gap-3 rounded-2xl bg-white px-4 py-3 shadow-md">
              ...
              <input
                ...
                placeholder="Search within blood tests..."
          ...
          <div>
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <p className="text-sm text-[#6b5a64]">
                Showing <span className="font-semibold">{filteredTests.length}</span>{" "}
                tests
              </p>
            ...
            <div className="mt-5 space-y-5">
              ...
              {filteredTests.length === 0 && (
                <div className="rounded-2xl border border-dashed border-gray-200 p-8 text-center text-sm text-gray-500">
                  No tests match your filters. Try clearing some filters.
                </div>
              )}
            </div>
          </div>
